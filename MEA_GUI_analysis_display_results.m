function MEA_GUI_analysis_display_results(AllDataRaw, num_well_rows, num_well_cols, beat_to_beat, analyse_all_b2b, stable_ave_analysis, spon_paced, well_electrode_data, Stims, added_wells, bipolar, save_dir)
    ...
    if strcmp(beat_to_beat, 'on')
        ...
        elseif strcmp(stable_ave_analysis, 'stable')
            ...
            accept_GE_button = uibutton(main_p,'push', 'BackgroundColor', '#3dd4d1','Text', 'Accept Golden Electrodes', 'Position', [screen_width-180 200 120 50], 'ButtonPushedFcn', @(accept_GE_button,event) acceptGEButtonPushed(accept_GE_button, out_fig, well_electrode_data(:)));
    ...
    function wellButtonPushed(well_button, added_wells, well_count, num_electrode_rows, num_electrode_cols, beat_to_beat, stable_ave_analysis, bipolar, spon_paced, out_fig)
        ...
        if strcmp(beat_to_beat, 'on')
            ...
            %save_button = uibutton(main_well_pan,'push',  'BackgroundColor', '#3dd4d1', 'Text', 'Save', 'Position', [screen_width-220 300 100 50], 'ButtonPushedFcn', @(save_button,event) saveB2BButtonPushed(save_button, electrode_data, save_dir, well_ID, num_electrode_rows, num_electrode_cols));
            ...
            if strcmp(stable_ave_analysis, 'time_region')
                ...
                %save_button = uibutton(main_well_pan,'push', 'BackgroundColor', '#3dd4d1', 'Text', 'Save', 'Position', [screen_width-220 300 100 50], 'ButtonPushedFcn', @(save_button,event) saveAveTimeRegionPushed(save_button, electrode_data, save_dir, well_ID, num_electrode_rows, num_electrode_cols));
            ...
            for elec_c = 1:num_electrode_cols
                ...
                if strcmp(beat_to_beat, 'on')
                    ...
                    if strcmp(stable_ave_analysis, 'time_region') 
                        ...
                        adv_stats_elec_button = uibutton(elec_pan,'push','Text', 'Advanced Results View', 'Position', [100 20 100 20], 'ButtonPushedFcn', @(adv_stats_elec_button,event) expandAveTimeRegionElectrodePushed(adv_stats_elec_button, electrode_data(electrode_count)));
        ...
        function rejectElectrodeButtonPushed(reject_electrode_button, num_electrode_rows, num_electrode_cols, elec_pan, electrode_count, undo_elec_pan)
            ...
            well_electrode_data(well_count,electrode_count).bdt = NaN;
            electrode_data(electrode_count).min_bp = NaN;
            well_electrode_data(well_count,electrode_count).min_bp = NaN;
            electrode_data(electrode_count).max_bp = NaN;
            well_electrode_data(well_count,electrode_count).max_bp = NaN;
            electrode_data(electrode_count).post_spike_hold_off = NaN;
            well_electrode_data(well_count,electrode_count).post_spike_hold_off = NaN;
            electrode_data(electrode_count).t_wave_offset = NaN;
            well_electrode_data(well_count,electrode_count).t_wave_offset = NaN;
            electrode_data(electrode_count).t_wave_duration = NaN;
            well_electrode_data(well_count,electrode_count).t_wave_duration = NaN;
            electrode_data(electrode_count).t_wave_shape = NaN;
            well_electrode_data(well_count,electrode_count).t_wave_shape = NaN;
            electrode_data(electrode_count).stim_spike_hold_off = NaN;
            well_electrode_data(well_count,electrode_count).stim_spike_hold_off = NaN;
            electrode_data(electrode_count).time_region_start = NaN;
            well_electrode_data(well_count,electrode_count).time_region_start = NaN;
            electrode_data(electrode_count).time_region_end = NaN;
            well_electrode_data(well_count,electrode_count).time_region_end = NaN;
            electrode_data(electrode_count).stable_beats_duration = NaN;
            well_electrode_data(well_count,electrode_count).stable_beats_duration = NaN;
            %}
        ...
        function reanalyseWellButtonPushed(reanalyse_well_button, well_elec_fig, num_electrode_rows, num_electrode_cols, well_pan, spon_paced, beat_to_beat, analyse_all_b2b, stable_ave_analysis)
            ...
            [well_electrode_data(well_count).electrode_data] = reanalyse_b2b_well_analysis(electrode_data, num_electrode_rows, num_electrode_cols, well_elec_fig, well_pan, spon_paced, beat_to_beat, analyse_all_b2b, stable_ave_analysis, well_ID);
        ...
        function reanalyseTimeRegionWellButtonPushed(reanalyse_well_button, well_elec_fig, num_electrode_rows, num_electrode_cols, well_pan, spon_paced, beat_to_beat, analyse_all_b2b, stable_ave_analysis)
            set(well_elec_fig, 'Visible', 'off')
            [well_electrode_data(well_count).electrode_data] = reanalyse_time_region_well(electrode_data, num_electrode_rows, num_electrode_cols, well_elec_fig, well_pan, spon_paced, beat_to_beat, analyse_all_b2b, stable_ave_analysis, well_ID);
            electrode_data = well_electrode_data(well_count).electrode_data;
        end
        ...
        function reanalyseButtonPushed(reanalyse_button, well_elec_fig, num_electrode_rows, num_electrode_cols, well_pan, spon_paced, beat_to_beat, analyse_all_b2b, stable_ave_analysis)
            ...
            function submitReanalyseButtonPushed(submit_reanalyse_button, well_elec_fig, reanalyse_fig, num_electrode_rows, num_electrode_cols, well_pan, spon_paced, beat_to_beat, analyse_all_b2b, stable_ave_analysis)
                ...
                [well_electrode_data(well_count).electrode_data] = electrode_analysis(electrode_data, num_electrode_rows, num_electrode_cols, reanalyse_electrodes, well_elec_fig, well_pan, spon_paced, beat_to_beat, analyse_all_b2b, stable_ave_analysis);
        ...
        function reanalyseTimeRegionButtonPushed(reanalyse_button, well_elec_fig, num_electrode_rows, num_electrode_cols, well_pan, spon_paced, beat_to_beat, analyse_all_b2b, stable_ave_analysis)
            ...
            function submitReanalyseButtonPushed(submit_reanalyse_button, well_elec_fig, reanalyse_fig, num_electrode_rows, num_electrode_cols, well_pan, spon_paced, beat_to_beat, analyse_all_b2b, stable_ave_analysis)
                ...
                [well_electrode_data(well_count).electrode_data] = electrode_time_region_analysis(electrode_data, num_electrode_rows, num_electrode_cols, reanalyse_electrodes, well_elec_fig, well_pan, spon_paced, beat_to_beat, analyse_all_b2b, stable_ave_analysis);
                %%disp(electrode_data(re_count).activation_times(2))
                electrode_data = well_electrode_data(well_count).electrode_data;
            end
        ...
        function heatMapButtonPushed(heat_map_button, well_elec_fig, well_ID, num_electrode_rows, num_electrode_cols, spon_paced)
            ...
            conduction_map_GUI2(start_activation_times, num_electrode_rows, num_electrode_cols, spon_paced, well_elec_fig)
        end
    ...
    function skipWellGEButtonPushed(reset_GE_button, added_wells, change_GE_dropdown, change_GE_text, change_GE_button)
        set(change_GE_button, 'Visible', 'on');
        set(change_GE_text, 'Visible', 'off');
        set(change_GE_dropdown, 'Visible', 'off');
        set(reset_GE_button, 'Visible', 'off');
            ...
            if num_button_rows > 1
                ...
                for ge_res_r = 1:num_button_rows
                   for ge_res_c = 1: num_button_cols
                       ...
                       electrode_data = well_electrode_data(ge_res_count).electrode_data;
                       if strcmp(get(drop_down, 'Visible'), 'off')
                            min_stdevs = [electrode_data(:).min_stdev];
                            non_zero_stddevs = find(min_stdevs ~=0);
                            min_electrode_beat_stdev_indx = find(min_stdevs == min(min_stdevs(non_zero_stddevs)), 1);
                ...
                for res_ge = 1:num_wells
                    ...
                    electrode_data = well_electrode_data(res_ge).electrode_data;
                    ...
                    plot(GE_res_ax, electrode_data(min_electrode_beat_stdev_indx).ave_min_depol_time, electrode_data(min_electrode_beat_stdev_indx).ave_min_depol_point, 'bo');
                    plot(GE_res_ax, electrode_data(min_electrode_beat_stdev_indx).ave_activation_time, electrode_data(min_electrode_beat_stdev_indx).average_waveform(electrode_data(min_electrode_beat_stdev_indx).ave_wave_time == electrode_data(min_electrode_beat_stdev_indx).ave_activation_time), 'go');
                    title(GE_res_ax, electrode_data(min_electrode_beat_stdev_indx).electrode_id,  'Interpreter', 'none')
                    peak_indx = find(electrode_data(min_electrode_beat_stdev_indx).ave_wave_time >= electrode_data(min_electrode_beat_stdev_indx).ave_t_wave_peak_time);
                    peak_indx = peak_indx(1);
                    t_wave_peak = electrode_data(min_electrode_beat_stdev_indx).average_waveform(peak_indx);
        ...
        for elec_r = num_electrode_rows:-1:1
            for elec_c = 1:num_electrode_cols
                ...
                if strcmp(spon_paced, 'spon')
                    ...
                    time_end_array = num2cell([electrode_data(electrode_count).time_region_end]);
                    time_end_array = vertcat('Time Region End (s)', time_end_array);
                    
                    bdt_array = num2cell([electrode_data(electrode_count).bdt]);
                    bdt_array = vertcat('Beat Detection Threshold Input (V)', bdt_array);
                    
                    min_bp_array = num2cell([electrode_data(electrode_count).min_bp]);
                    min_bp_array = vertcat('Mininum Beat Period Input (s)', min_bp_array);
                    
                    max_bp_array = num2cell([electrode_data(electrode_count).max_bp]);
                    max_bp_array = vertcat('Maximum Beat Period Input (s)', max_bp_array);
                    
                    post_spike_hold_off_array = num2cell([electrode_data(electrode_count).post_spike_hold_off]);
                    post_spike_hold_off_array = vertcat('Post-spike hold-off (s)', post_spike_hold_off_array);
                    
                    t_wave_duration_array = num2cell([electrode_data(electrode_count).t_wave_duration]);
                    t_wave_duration_array = vertcat('T-wave Duration Input (s)', t_wave_duration_array);
                    
                    t_wave_offset_array = num2cell([electrode_data(electrode_count).t_wave_offset]);
                    ...
                    warning_array = vertcat('Warnings', {electrode_data(electrode_count).ave_warning});
                ...
                elseif strcmp(spon_paced, 'paced')
                    ...
                    stim_spike_hold_off_array = num2cell([electrode_data(electrode_count).stim_spike_hold_off]);
    ...
    function saveAllB2BButtonPushed(save_button, save_dir, num_electrode_rows, num_electrode_cols)
        ...
        for w = 1:num_wells
            ...
            saveB2BButtonPushed(save_button, electrode_data, save_dir, well_ID, num_electrode_rows, num_electrode_cols);
    ...
    function saveAllTimeRegionButtonPushed(save_button, save_dir, num_electrode_rows, num_electrode_cols)
        ...
        for w = 1:num_wells
            well_ID = added_wells(w);
            electrode_data = well_electrode_data(w).electrode_data;
            saveAveTimeRegionPushed(save_button, electrode_data, save_dir, well_ID, num_electrode_rows, num_electrode_cols)
