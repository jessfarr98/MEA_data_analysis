function MEA_GUI(path_to_input, raw_file)
%  Create and then hide the UI as it is being constructed.
    
   close all hidden;
   close all;
   
   raw_file = fullfile(path_to_input, raw_file);
   ...
   plots_input_ui = uieditfield(start_pan,'Text','Position',[810 480 140 25], 'ValueChangedFcn',@(plots_input_ui,event) changePlotsDir(plots_input_ui, start_fig, path_to_input, run_button, run_fast_button));
   ...
   function runButtonPushed(run_button, RawData,RawFileData, Stims, b2b_options_dropdown, stable_options_dropdown, b2bdropdown, paced_spon_options_dropdown, start_fig, bipolarDropdown, plate_well_options_dropdown, path_to_input)
      %disp('worked')
      ...
      if plate_well_options_dropdown.Value == 1
          MEA_BDT_GUI_V2(RawData,Stims, beat_to_beat, spon_paced, analyse_all_b2b, stable_ave_analysis, added_wells, bipolar, fullfile(path_to_input, get(plots_input_ui, 'Value')))
      
      else
          MEA_BDT_PLATE_GUI_V2(RawData,Stims, beat_to_beat, spon_paced, analyse_all_b2b, stable_ave_analysis, added_wells, bipolar, fullfile(path_to_input, get(plots_input_ui, 'Value')))
      
      end
          
   end

   function runFastButtonPushed(run_fast_button, RawData,RawFileData, Stims, b2b_options_dropdown, stable_options_dropdown, b2bdropdown, paced_spon_options_dropdown, start_fig, bipolarDropdown, plate_well_options_dropdown, path_to_input)
      %disp('worked')
      %disp(b2bdropdown.Value);
      %disp(stable_options_dropdown.Value);
      %disp(b2b_options_dropdown.Value);
      ...
      if plate_well_options_dropdown.Value == 1
          MEA_GUI_FAST_THRESHOLD_INPUTS(RawData, start_fig, Stims, beat_to_beat, spon_paced, analyse_all_b2b, stable_ave_analysis, added_wells, bipolar, fullfile(path_to_input, get(plots_input_ui, 'Value')))
   ...
   function changePlotsDir(plots_input_ui, start_fig, path_to_input, run_button, run_fast_button)
        prompt_user(plots_input_ui, 'dir', path_to_input, start_fig, run_button, run_fast_button);
   end
