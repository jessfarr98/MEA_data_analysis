...
%if found_electrode == 1
    ...
    elseif strcmp(electrode_data(electrode_count).spon_paced, 'paced')
        ...
        if end_indx == length(Stims)
            stim_end_indx = end_indx;
           
        else
            stim_end_indx = end_indx+1;
        end
        [beat_num_array, cycle_length_array, activation_times, activation_point_array, beat_start_times, beat_periods, t_wave_peak_times, t_wave_peak_array, max_depol_time_array, min_depol_time_array, max_depol_point_array, min_depol_point_array, depol_slope_array, warning_array, Stim_volts filtered_time, filtered_data] = extract_paced_beats('', electrode_data(electrode_count).time(start_plot_indx:end_plot_indx), electrode_data(electrode_count).data(start_plot_indx:end_plot_indx), NaN, electrode_data(electrode_count).spon_paced, 'on', 'all', NaN, NaN, stable_ave_analysis, NaN, NaN, '', electrode_data(electrode_count).electrode_id, t_wave_shape, get(t_wave_duration_ui, 'Value'), electrode_data(electrode_count).Stims(start_indx:stim_end_indx), get(post_spike_ui, 'Value'), get(stim_spike_ui, 'Value'), get(t_wave_peak_offset_ui, 'Value'),nan, filter_intensity, 'on');     
    ...
    if filtered_start_indx ~= 1
        ...
        if end_plot_indx ~= length(electrode_data(electrode_count).filtered_data)
            ...
            electrode_data(electrode_count).filtered = [electrode_data(electrode_count).filtered_data(1:filtered_start_indx-1) filtered_data electrode_data(electrode_count).filtered_data(filtered_end_indx+1:end)];
            ...
            electrode_data(electrode_count).filtered = [electrode_data(electrode_count).filtered_data(1:filtered_start_indx-1) filtered_data];
        ...
        if end_plot_indx ~= length(electrode_data(electrode_count).filtered_data)
            ...
            electrode_data(electrode_count).filtered = [filtered_data electrode_data(electrode_count).filtered_data(filtered_end_indx+1:end)];
            
        else
            electrode_data(electrode_count).filtered_time = filtered_time;
            electrode_data(electrode_count).filtered = filtered_data;
