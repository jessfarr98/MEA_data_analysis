           ...
           for e_r = 1:num_electrode_rows
              for e_c = 1:num_electrode_cols
                 ...
                 if (strcmp(class(RawWellData),'Waveform') && strcmp(class(ArtElimWellData),'Waveform'))
                     ...
                     bdt = 1.2e-3;
                     [raw_beat_num_array, raw_cycle_length_array, raw_activation_time_array, raw_beat_start_times, raw_beat_periods, raw_depol_array, raw_FPD_array, raw_T_wave_times] = extract_beats(wellID, raw_time, raw_data, bdt, 'paced', 'on', 'all', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', electrode_id);
                     [AE_beat_num_array, AE_cycle_length_array, AE_activation_time_array, AE_beat_start_times, AE_beat_periods, AE_depol_array, AE_FPD_array, AE_T_wave_times] = extract_beats(wellID, AE_time, AE_data, bdt, 'paced', 'on', 'all', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', electrode_id);

                     compare_raw_elim_beats(raw_data, raw_time, AE_data, AE_time, raw_beat_num_array, raw_cycle_length_array, raw_activation_time_array, raw_beat_start_times, raw_beat_periods, raw_depol_array, raw_FPD_array, raw_T_wave_times, AE_beat_num_array, AE_cycle_length_array, AE_activation_time_array, AE_beat_start_times, AE_beat_periods, AE_depol_array, AE_FPD_array, AE_T_wave_times, fullfile('Compare Stimulus Eliminated', 'Compare paced paced SP 600us(001))', electrode_id), directory, electrode_id)
...
function compare_raw_elim_beats(raw_data, raw_time, AE_data, AE_time, raw_beat_num_array, raw_cycle_length_array, raw_activation_time_array, raw_beat_start_times, raw_beat_periods, raw_depol_array, raw_FPD_array, raw_T_wave_times, AE_beat_num_array, AE_cycle_length_array, AE_activation_time_array, AE_beat_start_times, AE_beat_periods, AE_depol_array, AE_FPD_array, AE_T_wave_times, save_plot, protocol, electrode_id)
    ...
    for AE_beat = 2:num_AE_beats
        ...
        act_time = AE_activation_time_array(AE_beat);
        act_signal = AE_data(find(AE_time == act_time));
        ...
        %plot_AE_time = AE_time(find(AE_time == prev_beat_time):find(AE_time == beat_time)) + (AE_activation_time_array(1)-raw_activation_time_array(2));
        plot_AE_time = AE_time(find(AE_time == prev_beat_time):find(AE_time == beat_time)) + (raw_beat_start_times(2)-AE_beat_start_times(2));
