function MEA_GUI_analysis_display_resultsV2(AllDataRaw, num_well_rows, num_well_cols, beat_to_beat, analyse_all_b2b, stable_ave_analysis, spon_paced, well_electrode_data, Stims, added_wells, bipolar, save_dir, reanalysis)
            ...
            for elec_c = 1:num_electrode_cols
                ...
                if strcmp(beat_to_beat, 'on')
                    ...
                    elseif strcmp(well_electrode_data(well_count).electrode_data(electrode_count).spon_paced, 'paced bdt')
                        %num_beats = length(well_electrode_data(well_count).electrode_data(electrode_count).beat_start_times);
                        [sr, sc] = size(well_electrode_data(well_count).electrode_data(electrode_count).Stims);
                        [br, bc] = size(well_electrode_data(well_count).electrode_data(electrode_count).beat_start_times);

                        if bc == 1 && sc ~= 1
                            well_electrode_data(well_count).electrode_data(electrode_count).beat_start_times = reshape(well_electrode_data(well_count).electrode_data(electrode_count).beat_start_times, [bc br]);
                        end
                        if sc == 1 && bc ~= 1
                            well_electrode_data(well_count).electrode_data(electrode_count).beat_start_times = reshape(well_electrode_data(well_count).electrode_data(electrode_count).beat_start_times, [bc br]);
                        end
                    ...
                    if num_beats > 4    
                        ...
                        if strcmp(well_electrode_data(well_count).electrode_data(electrode_count).spon_paced, 'paced')
                            ...
                            %if reanalysis == 0
                            filtered_time_reg_start_indx = find(well_electrode_data(well_count).electrode_data(electrode_count).filtered_time >= time_start);
                            filtered_time_reg_end_indx = find(well_electrode_data(well_count).electrode_data(electrode_count).filtered_time >= time_end);

                            plot(elec_ax,well_electrode_data(well_count).electrode_data(electrode_count).filtered_time(filtered_time_reg_start_indx(1):filtered_time_reg_end_indx(1)), well_electrode_data(well_count).electrode_data(electrode_count).filtered_data(filtered_time_reg_start_indx(1):filtered_time_reg_end_indx(1)));
                            ...
                            plot(elec_ax, well_electrode_data(well_count).electrode_data(electrode_count).beat_start_times(mid_beat), well_electrode_data(well_count).electrode_data(electrode_count).beat_start_volts(mid_beat), 'm.', 'MarkerSize', 20);
                        ...
                        if strcmp(well_electrode_data(well_count).electrode_data(electrode_count).spon_paced, 'paced')
                            ...
                            plot(elec_ax, well_electrode_data(well_count).electrode_data(electrode_count).beat_start_times, well_electrode_data(well_count).electrode_data(electrode_count).beat_start_volts, 'm.', 'MarkerSize', 20);
