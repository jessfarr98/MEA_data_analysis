function MEA_BDT_GUI_V2(RawData,Stims, beat_to_beat, spon_paced, analyse_all_b2b, stable_ave_analysis, added_wells, bipolar, save_dir, save_base_dir, minimise_wells, constant_signal, origin_well)
...
% QC with average FPD input from user that then does another t-wave peak picking method if fails
   ...
   for w_r = 1:num_well_rows
       for w_c = 1:num_well_cols
          ...
          ruler_label = uieditfield(well_p,'Text', 'FontSize', 8,'Value', 'Distance Ruler (s)', 'Position', [screen_width-200 570 100 60], 'Editable','off');
          ruler_value = uieditfield(well_p, 'numeric', 'Position', [screen_width-200 510 100 60], 'Value', 0, 'editable', 'off');
   ...
   function txt = DistanceRulerCallback(~, info)
        %EventData.IntersectionPoint
        
        curr_x = info.Position(1);
        yt = info.Position(2);
        
        txt = ['(' num2str(curr_x) ', ' num2str(yt) ')'];
        ...
        if ~isnan(prev_x)
            ...
            set(ruler_value, 'value', distance)
