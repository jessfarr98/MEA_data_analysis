function [activation_time, amplitude, max_depol_time, max_depol_point, min_depol_time, min_depol_point, slope] = rate_analysis(time, data, post_spike_hold_off, stim_spike_hold_off, spon_paced, stim_time, electrode_id)
    ...
    %{
    if min_raw_slope < -1*max_abs_slope
        activation_time_indx = find(depol_complex_data_derivative == min_raw_slope);
    else
        activation_time_indx = find(abs(depol_complex_data_derivative) == max_abs_slope);
    end
    %}
    ...
    min_raw_act_time = depol_complex_time(activation_time_indx_min_raw(1));
    max_raw_act_time = depol_complex_time(activation_time_indx_max_raw(1));
    max_abs_act_time = depol_complex_time(activation_time_indx_max_abs(1));
    ...
    if ((max_depol_time >= min_raw_act_time)&& (min_raw_act_time>= min_depol_time)) || ((min_depol_time >= min_raw_act_time) && (min_raw_act_time>= max_depol_time))
        ...
        if ((max_depol_time >= max_abs_act_time)&& (max_abs_act_time>= min_depol_time)) || ((min_depol_time >= max_abs_act_time)&& (max_abs_act_time>= max_depol_time))
            ...
            if ((max_depol_time >= max_raw_act_time)&& (max_raw_act_time>= min_depol_time)) || ((min_depol_time >= max_raw_act_time)&& (max_raw_act_time>= max_depol_time))
                activation_time = depol_complex_time(activation_time_indx_max_raw(1));
                %slope = depol_complex_data(activation_time_indx(1)); %3/12/2021 bug was calculating the slope using the original y data, not the dydx of te signal - this is a scalar value
