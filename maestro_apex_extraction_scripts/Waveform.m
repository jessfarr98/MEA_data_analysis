classdef Waveform
    ...
    methods
        ...
        function voltageData = GetVoltageVector(this)
            ...
            %fake_time = linspace(0, 1, length(fData));
            
            %plot(fake_time, fData)
            fSource = [this(:).Source];
            fHeader = [fSource(:).Header];
            fVoltageScale = [fHeader(:).VoltageScale];
            %disp(fVoltageScale*max(fData))
            %disp(diag(fVoltageScale))
            voltageData = fData * diag(fVoltageScale);
        end
        
        %{
        %mofidication by Jessica Farr
        function SetVoltageVector(this, voltageData)
            ...
            this(:).Data = voltageData/diag(fVoltageScale);
        end
        %}
